...
class TestMigrateData:
    ...
    def test_preserves_repo_root_if_present(self):
        data = {"repo_root": "/new/path", "worktree_base": "/wt"}
        migrated = _migrate_data(data)
        assert migrated["repo_root"] == "/new/path"

    def test_repo_root_takes_precedence(self):
        data = {"repo_root": "/new", "repo_path": "/old", "worktree_base": "/wt"}
        migrated = _migrate_data(data)
        # repo_root already exists, repo_path stays but is ignored by model
...
class TestLoadAndSaveState:
    ...
    def test_save_and_load_roundtrip(self, fake_config):
        ...
        assert len(loaded.worktrees[0].sessions) == 1
...
class TestRemoveWorktreeFromState:
    def test_removes_matching_worktree(self):
        ...
        assert result.worktrees[0].name == "b"

    def test_no_op_for_missing_name(self):
...
class TestRemoveSessionFromState:
    def test_removes_matching_session(self):
        ...
        assert len(result.worktrees[0].sessions) == 1
        assert result.worktrees[0].sessions[0].id == "bbb"

    def test_no_op_for_missing_worktree(self):
...
class TestReconcileState:
    def test_prunes_missing_paths(self, tmp_path, monkeypatch):
        ...
        # Mock prune_git_cache to avoid side effects
        ...
        assert state.worktrees[0].name == "exists"
    ...
    def test_no_change_when_all_exist(self, tmp_path, monkeypatch):
        ...
        d2 = tmp_path / "b"
        d2.mkdir()
        wt1 = Worktree(name="a", path=str(d1), branch="sw-a")
        wt2 = Worktree(name="b", path=str(d2), branch="sw-b")
        state = AppState(
            repo_root=str(tmp_path),
            worktree_base=str(tmp_path),
            worktrees=[wt1, wt2],
...
class TestProjectsRegistry:
    @pytest.mark.usefixtures("_redirect_state_dir")
    def test_update_and_load(self, fake_config):
        update_projects_registry(fake_config)
        projects = load_projects_registry()
        assert str(fake_config.repo_root) in projects

    @pytest.mark.usefixtures("_redirect_state_dir")
    def test_no_duplicates(self, fake_config):
        update_projects_registry(fake_config)
        update_projects_registry(fake_config)
        projects = load_projects_registry()
...
class TestRecoverDeadSessions:
    def test_no_dead_sessions_is_noop(self, tmp_path, monkeypatch):
        ...
        changed = recover_dead_sessions(state)
        assert changed is False
    ...
    def test_dead_sessions_replaced_with_resumed(self, tmp_path, monkeypatch):
        ...
        assert created_sessions[0]["resume"] is True
        assert created_sessions[0]["skip_permissions"] is False
